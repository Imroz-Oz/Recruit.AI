    ...
    <div className="space-y-10 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-300/5 pb-10">
        <div className="space-y-4">
          ...
          <p className="text-[#0f172a]/60 font-medium text-base max-w-lg leading-relaxed">
        ...
        <div className="flex items-center gap-3">
          <button className="px-5 py-3 bg-white border border-slate-300/5 rounded-2xl flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-[#0f172a]/60 hover:border-indigo-electric/20 transition-all">
            <Calendar className="w-4 h-4" /> Last 30 Days
          </button>
          <button className="px-5 py-3 bg-[#1e293b] text-white rounded-2xl flex items-center gap-2 text-sm font-bold uppercase tracking-widest hover:bg-indigo-electric transition-all shadow-xl shadow-midnight/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: 'Interviews/Offer', val: '3.8', delta: 'Valid', trend: 'neutral', icon: Clock, color: 'text-[#0f172a]', bg: 'bg-[#1e293b]/10' },
        ].map((stat, i) => (
          <motion.div 
            ...
            key={stat.label}
            className="bg-white p-8 rounded-3xl border border-slate-300/5 shadow-sm hover:shadow-xl hover:shadow-midnight/5 transition-all group"
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <div className={cn(
                "px-2 py-1 rounded-full text-base font-bold flex items-center gap-1",
                stat.trend === 'up' ? "bg-emerald-50 text-emerald-600" : stat.trend === 'down' ? "bg-red-50 text-red-600" : "bg-warm-gray text-[#0f172a]/40"
              )}>
            ...
            <h4 className="text-3xl font-serif font-bold text-[#0f172a] italic">{stat.val}</h4>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Main Growth Area */}
        <div className="lg:col-span-2 bg-white p-10 rounded-3xl border border-slate-300/5 shadow-sm space-y-8">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-2xl font-serif font-bold italic text-[#0f172a]">Mission Trajectory</h3>
              <p className="text-base font-bold uppercase tracking-[0.2em] text-[#0f172a]/30">Placements vs Outreach Intensity</p>
            </div>
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-indigo-electric" />
                <span className="text-base font-bold text-[#0f172a]/40 uppercase">Placements</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-indigo-electric/30" />
                <span className="text-base font-bold text-[#0f172a]/40 uppercase">Interviews</span>
              </div>
            </div>
          </div>
        ...
        {/* Source ROI */}
        <div className="bg-[#1e293b] p-10 rounded-3xl shadow-2xl space-y-8 flex flex-col">
          <div>
            <h3 className="text-2xl font-serif font-bold italic text-white">Sourcing DNA</h3>
            <p className="text-base font-bold uppercase tracking-[0.2em] text-white/40">Channel Yield ROI</p>
          </div>
              ...
              {sourceData.map((s) => (
                <div key={s.name} className="flex items-center justify-between">
                   <div className="flex items-center gap-3">
                     ...
                     <span className="text-sm font-bold text-white/60 uppercase tracking-widest">{s.name}</span>
                   </div>
      ...
      <div className="bg-white/40 backdrop-blur-sm p-10 md:p-16 rounded-[4rem] border border-white shadow-sm overflow-hidden relative">
        ...
        <div className="relative z-10 space-y-12">
          <div className="text-center max-w-2xl mx-auto space-y-4">
            <h3 className="text-4xl font-serif font-bold italic text-[#0f172a] italic">Conversion Dynamics</h3>
            <p className="text-base font-medium text-[#0f172a]/40 leading-relaxed">
          ...
          <div className="flex flex-col md:flex-row items-end justify-between gap-4 h-[300px]">
             {conversionData.map((stage, i) => (
               <div key={stage.stage} className="flex-1 flex flex-col items-center gap-4 h-full justify-end group">
                 <div className="text-base font-bold text-[#0f172a]/30 mb-2 uppercase tracking-widest">{stage.count}</div>
                 <motion.div 
                   initial={{ height: 0 }}
                 ...
                 <div className="text-base font-bold uppercase tracking-[0.2em] text-[#0f172a] pt-4 mt-auto">{stage.stage}</div>
               </div>
             ))}
          </div>
        </div>
