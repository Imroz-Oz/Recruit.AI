    ...
    <div className="max-w-6xl mx-auto space-y-12 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/5 border border-amber-100">
            ...
            <span className="text-base font-bold uppercase tracking-[0.2em] text-amber-500">
              Readiness Simulation Protocol
            </span>
          </div>
          <h2 className="text-5xl font-serif font-bold text-[#0f172a] italic">Readiness Engine</h2>
        ...
        <div className="flex gap-4">
          <div className="px-6 py-3 bg-white rounded-2xl border border-slate-300/5 shadow-sm flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="text-base font-bold uppercase tracking-widest text-[#0f172a]/30">Prep Level</p>
              <p className="text-base font-serif font-bold text-[#0f172a] italic">Advanced Elite</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="relative aspect-video bg-[#1e293b] rounded-3xl overflow-hidden shadow-2xl shadow-midnight/40 group">
              ...
              {!sessionStarted ? (
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-white/40 text-base max-w-sm mx-auto font-medium">Session will focus on "Senior Technical Leadership" and "Full-Stack System Design".</p>
                  </div>
                  <button 
                    onClick={toggleSession}
                    className="flex items-center gap-4 px-10 py-5 bg-white text-[#0f172a] rounded-2xl text-base font-bold uppercase tracking-[0.3em] hover:bg-amber-500 hover:text-white transition-all group/btn"
                  >
                ...
                >
                  <div className="flex justify-between items-start mb-auto">
                    <div className="flex items-center gap-4 bg-white/5 border border-white/10 px-4 py-2 rounded-full">
                       ...
                       <span className="text-base font-bold uppercase tracking-widest text-white/60">Live Analysis Tracking</span>
                    </div>
                    <button 
                      onClick={toggleSession}
                  ...
                  <div className="max-w-2xl mx-auto text-center space-y-4">
                     <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/20 border border-amber-500/40 rounded-full">
                        <span className="text-base font-bold uppercase tracking-widest text-amber-500">Question {currentQuestion + 1} of {questions.length}</span>
                     </div>
                     ...
                     <div className="w-full text-left mt-4 text-white">
                        <textarea
                          ...
                          className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-base outline-none focus:border-indigo-electric/50 transition-colors h-32 resize-none"
                        ></textarea>
                     </div>
                     ...
                     {evaluation && (
                       <div className="text-left bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-xl space-y-2 mt-4 text-white animate-in fade-in slide-in-from-bottom-2">
                         <div className="flex justify-between items-center text-emerald-400">
                           <span className="text-base font-bold uppercase">AI Evaluation</span>
                           <span className="text-xl font-black">{evaluation.score}/100</span>
                         </div>
                         <p className="text-base">{evaluation.feedback}</p>
                         <p className="text-base text-emerald-200/60 italic overflow-hidden line-clamp-2 hover:line-clamp-none transition-all">Model Answer: {evaluation.modelAnswer}</p>
                       </div>
                     )}
                  </div>

                  <div className="mt-auto flex items-center justify-center gap-4">
                     <button onClick={handleEvaluate} disabled={isEvaluating || !answer} className="flex-1 py-4 bg-indigo-electric hover:bg-indigo-600 text-white rounded-xl text-base font-bold uppercase disabled:opacity-50 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="bg-white p-8 rounded-3xl border border-slate-300/5 space-y-6">
                <h5 className="text-base font-bold uppercase tracking-[0.3em] text-[#0f172a]/20">Confidence Meter</h5>
                ...
                <p className="text-base font-medium text-[#0f172a]/40 leading-relaxed italic">
                   "Your speech cadence is optimal. Watch for filler words like 'basically' during technical explanations."
                </p>
             </div>
             <div className="bg-white p-8 rounded-3xl border border-slate-300/5 space-y-6">
                <h5 className="text-base font-bold uppercase tracking-[0.3em] text-[#0f172a]/20">Key Competency focus</h5>
                <div className="flex flex-wrap gap-2">
                   {['Scalability', 'Reliability', 'Strategic Influence', 'Conflict Resolution'].map(tag => (
                      <span key={tag} className="px-3 py-1 bg-indigo-electric/5 text-indigo-electric rounded-lg text-base font-bold uppercase tracking-widest">
                        {tag}
                      </span>
                   ))}
                </div>
        ...
        <aside className="space-y-8">
          <div className="bg-white p-8 rounded-3xl border border-slate-300/5 shadow-sm space-y-8">
              <div className="space-y-2">
                <p className="text-base font-bold uppercase tracking-widest text-[#0f172a]/20">Active Scenario</p>
                <div className="flex justify-between items-center">
                  <h4 className="text-2xl font-serif font-bold text-[#0f172a] italic">{scenario}</h4>
                  <button 
                  ...
                  >
                    {isGenerating ? <Loader2 className="w-4 h-4 animate-spin text-indigo-electric" /> : <RefreshCw className="w-4 h-4 text-[#0f172a]/40" />}
                  </button>
                </div>
              </div>
             
             <div className="space-y-4">
                <h5 className="text-base font-bold uppercase tracking-[0.3em] text-[#0f172a]/20">Evaluation Matrix</h5>
             ...
             <button className="w-full py-5 bg-[#1e293b] text-white rounded-2xl text-base font-bold uppercase tracking-[0.2em] hover:bg-emerald-600 transition-all shadow-xl shadow-midnight/10 flex items-center justify-center gap-3">
                Download Analysis Report <Zap className="w-4 h-4 fill-current text-white/50" />
             </button>
          </div>

          <div className="bg-emerald-900 p-8 rounded-3xl text-white relative overflow-hidden group cursor-pointer">
             <div className="relative z-10 space-y-4">
                <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center">
                  <Target className="w-5 h-5 text-white/60" />
                </div>
                <h4 className="text-xl font-serif font-bold italic">Optimize Your Answers</h4>
                <p className="text-white/40 text-base leading-relaxed font-medium">Use the "STAR+" framework to boost your leadership indicators by 25%.</p>
                <div className="flex items-center gap-2 text-base font-bold uppercase tracking-widest text-emerald-400 group-hover:translate-x-1 transition-transform">
